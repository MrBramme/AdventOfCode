...
namespace AdventOfCode.Year2022.Solutions
{
    public class Solution202205Part2 : ISolution
    {
        ...
        public string GetSolution()
        {
            ...
            {
                var cratesBeingMoved = new char[movement.Amount];
                for (var i = 0; i < movement.Amount; i++)
                {
                    var crate = crates[movement.Source - 1].Pop();
                    cratesBeingMoved[movement.Amount -1 - i] = crate;
                }
                foreach (var crateBeingMoved in cratesBeingMoved)
                    crates[movement.Target - 1].Push(crateBeingMoved);
            }
