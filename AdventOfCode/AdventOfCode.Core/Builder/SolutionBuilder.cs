    ...
    public class SolutionBuilder : ISolutionBuilder
    {
        ...
        public ISolution CreateSolution(Assignment assignment)
        {
            return assignment.Year switch
            {
                ...
                2022 => Get2022Solution(assignment),
                2023 => Get2023Solution(assignment),
        ...
        private ISolution Get2022Solution(Assignment assignment)
        {
            var solutions2022 = Assembly.GetAssembly(typeof(Assembly2022))
                .GetTypes()
                .Where(x => x.GetInterfaces().Contains(typeof(ISolution)))
                .Where(x => x.IsClass);

            return (ISolution)_serviceProvider.GetService(solutions2022.First(s => s.Name.Equals(GetSolutionName(assignment))));
        }

        private ISolution Get2023Solution(Assignment assignment)
        {
            var solutions2023 = Assembly.GetAssembly(typeof(Assembly2023))
                .GetTypes()
                .Where(x => x.GetInterfaces().Contains(typeof(ISolution)))
                .Where(x => x.IsClass);

            return (ISolution)_serviceProvider.GetService(solutions2023.First(s => s.Name.Equals(GetSolutionName(assignment))));
        }
