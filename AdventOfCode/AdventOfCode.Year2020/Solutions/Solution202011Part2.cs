...
{
    public class Solution202011Part2 : ISolution
    {
        private readonly ILogger<Solution202011Part2> _logger;
        private readonly IInputService _inputService;
        private readonly string resourceLocation = "Resources2020\\Day11.txt";
        public Solution202011Part2(ILogger<Solution202011Part2> logger, IInputService inputService)
        {
            _logger = logger;
            ...
            for (var row = 0; row < rowCount; row++)
            {
                for (var col = 0; col < colCount; col++)
                {
                    ...
                    if (currentSpot.IsSeat)
                    {
                        ...
                        if (currentSpot.IsOccupied && occupiedSeats >= 5)
                        {
        ...
        private static Spot? GoToSpot(Spot[][] input, int col, int colCount, int colStep, int row, int rowCount, int rowStep)
        {
            var isDone = false;
            var steps = 0;
            Spot result = null;
            ...
            {
                steps++;
                var newCol = col + (steps * colStep);
                var newRow = row + (steps * rowStep);
                var positionValid = newCol >= 0 && newCol < colCount && newRow >= 0 && newRow < rowCount;
                if (positionValid)
                {
                    if (input[newRow][newCol].IsSeat)
                    {
                        result = input[newRow][newCol];
                        isDone = true;
                    }
                }
                else
        ...
        private static List<Spot> GetAdjacentSpots(Spot[][] input, int col, int colCount, int row, int rowCount)
        {
            ...
            var leftSeat = GoToSpot(input, col, colCount, -1, row, rowCount, 0);
            if (leftSeat != null) adjacentSpots.Add(leftSeat);

            var rightSeat = GoToSpot(input, col, colCount, 1, row, rowCount, 0);
            if (rightSeat != null) adjacentSpots.Add(rightSeat);

            var TopLeftSeat = GoToSpot(input, col, colCount, -1, row, rowCount, 1);
            if (TopLeftSeat != null) adjacentSpots.Add(TopLeftSeat);

            var TopSeat = GoToSpot(input, col, colCount, 0, row, rowCount, 1);
            if (TopSeat != null) adjacentSpots.Add(TopSeat);

            var TopRightSeat = GoToSpot(input, col, colCount, 1, row, rowCount, 1);
            if (TopRightSeat != null) adjacentSpots.Add(TopRightSeat);

            var BottomLeftSeat = GoToSpot(input, col, colCount, -1, row, rowCount, -1);
            if (BottomLeftSeat != null) adjacentSpots.Add(BottomLeftSeat);

            var BottomSeat = GoToSpot(input, col, colCount, 0, row, rowCount, -1);
            if (BottomSeat != null) adjacentSpots.Add(BottomSeat);

            var BottomRightSeat = GoToSpot(input, col, colCount, 1, row, rowCount, -1);
            if (BottomRightSeat != null) adjacentSpots.Add(BottomRightSeat);
